...
Lead with the delivery outcome, then tasks and final verification. Give each task a heading and a
short explanation of what will work, followed by the affected code and tests, behavior to preserve,
and a command or concrete manual scenario with its expected result. Keep acceptance links and real
dependencies visible; group tasks into phases only when each phase has a verifiable outcome. Reference
existing authoritative contracts rather than copying them; without a spec, define necessary contracts
in the plan. Include a compact signature, payload, or pseudocode example when it resolves ambiguity;
keep full implementations in code. For difficult structure or flow, consult the `choosing-visuals` skill.

New plans start as `draft`. When amending, read the current plan and affected implementation,
preserve IDs and append new ones. Reopen tasks or criteria whose prior evidence no longer covers
the revised requirement. Reassess readiness and recompute affected task order and coverage when
the source design, scope, or dependencies change; do not reset completed work unrelated to the
amendment.
...
- The delivery outcome has tasks, every acceptance criterion maps to a task and final verification,
  and required outcomes have no unresolved material design question. Spec-backed coverage agrees with
...
- Consequential decisions and assumptions are recorded where needed; new dependencies or
  abstractions have a concrete benefit.
...
reconcile state with the worktree after interruptions.
